...
int main()
{
    ...
    if( bind(socket_desc,(struct sockaddr *)&addr1 , sizeof(addr1)) < 0)
    {
    ...
    puts("Waiting for incoming connections...");
        ...
        client_sock = accept(socket_desc, (struct sockaddr *)&socket_desc, (socklen_t*)&c);
        puts("Connection accepted");


        
        while(1){
        int n = write(client_sock,"Hello from Server\n",19);
        //recvPacket_pod(socket_desc);
            if(n<0){
                perror("Error!!");
            }
        int rc  = read(client_sock,buffer,256);
        printf("%s\n",buffer );
        }
